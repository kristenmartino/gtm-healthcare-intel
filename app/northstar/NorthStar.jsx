...
const METRIC_REGISTRY = [
  ...
  {
    name: "Practice Activation",
    category: "Customer Onboarding",
    definition: "A newly-sold practice is 'Activated' when it submits its first paid claim OR logs 10+ patient encounters, whichever comes first, within 30 days of contractual go-live. Designed to distinguish meaningfully-using accounts from contract-only handoffs that look healthy in CRM but never bill.",
    formula: "FLAG_ACTIVATED = (first_paid_claim_date - go_live_date) <= 30 OR encounter_count_30d_post_go_live >= 10",
    source: "Implementation Tracker + Billing System + EHR Encounter Log",
    owner: "Implementation Ops",
    refresh: "Daily (6:00 AM ET)",
    qualityScore: 76,
    lastValidated: "2025-02-22",
    validatedBy: "Emily Okafor, Implementation Ops",
    status: "disputed",
    conflicts: [
      { team: "Sales", altDef: "Contract signed = activated (used for quarterly rep comp credit)", resolved: false },
      { team: "Finance", altDef: "First paid claim submitted = activated (revenue recognition trigger)", resolved: false },
      { team: "Product", altDef: "Credentialing + sandbox provisioning complete = activated (technically usable)", resolved: false },
      { team: "CS", altDef: "First 30-day revenue threshold met = activated (best leading indicator of 12-month retention)", resolved: false },
    ],
  },
  {
